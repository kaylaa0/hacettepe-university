...
function render(gl, program, mode, NumVertices, vPositions, vColors) {
  // Render function, my first attempt so can be simplified further I assume.
  // Sourced from the TA's example from the lesson.
...
function populateColor(vColors, color, NumVertices) {
  for (var i = 0; i < NumVertices; i++) {
...
function main() {
  ...
  const program = initShaderProgram(gl, vShader, fShader); // Initializing the shader program.
  const buffer = gl.createBuffer(); // Creating buffer.
  ...
  populateColor(vColors, new color4(0.93, 0.83, 0.09, 1.0), NumVertices); // We manually populate the color array. Again this seems naive approach but I wanted to keep it simple
  render(gl, program, gl.TRIANGLE_FAN, NumVertices, vPositions, vColors); // Rendering.
  ...
  // Populate vertices.
  populateColor(vColors, new color4(0.84, 0.88, 0.92, 1.0), NumVertices);
  // Populate colors.
  render(gl, program, gl.TRIANGLE_FAN, NumVertices, vPositions, vColors);
  ...
  populateColor(vColors, new color4(0.23, 0.21, 0.08, 1), NumVertices);
  render(gl, program, gl.TRIANGLE_FAN, NumVertices, vPositions, vColors);
