...
class InputManager {
  ...
  static pointerLockMoveCallback(e) {
    ...
    var rotationMagn = Vector3.dot(
      KaylaEngine.camera.target.getNormalized(),
      [0.0, -1.0, 0.0]
    );
    ...
    if (
      ...
      if (KaylaEngine.camera.transform.rotation.z !== 0) {
        KaylaEngine.camera.transform.rotation.x = -Math.abs(
      ...
      if (KaylaEngine.camera.transform.rotation.x !== 0) {
        KaylaEngine.camera.transform.rotation.z = -Math.abs(
          KaylaEngine.camera.transform.rotation.x
        );
