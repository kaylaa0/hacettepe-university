...
class Renderer {
  ...
  static _Init() {
    Renderer._constants["resolution"] = Renderer.CalculateResolution();
  }

  static ResizeWindow() {
    // Resizing window.
    Renderer._constants["resolution"] = Renderer.CalculateResolution();
    Renderer.GL._SetSize(
      Renderer._constants["resolution"][0],
      Renderer._constants["resolution"][1]
    );
  }

  static CalculateResolution() {
    // Tried to lock 16:9
    if ((window.innerWidth - 10) / 16 <= (window.innerHeight - 70) / 9) {
      return [
        window.innerWidth - 10,
        Math.round(((window.innerWidth - 10) / 16) * 9),
        0,
        0,
      ];
    } else {
      return [
        Math.round(((window.innerHeight - 70) / 9) * 16),
  ...
  static setUpGL(canvasID) {
    // Setting up webgl2 canvas.
    ...
    // Methods
    // Reset the canvas with the set background color.
    gl._Clear = function () {
      this.clear(this.COLOR_BUFFER_BIT | this.DEPTH_BUFFER_BIT);
      return this;
    };

    // Create and fill an Array buffer.
    ...
    gl.fLoadTexture = function (name, img, doYFlip) {
      ...
      this.texParameteri(this.TEXTURE_2D, this.TEXTURE_WRAP_S, this.REPEAT); //Stretch image to X position
      this.texParameteri(this.TEXTURE_2D, this.TEXTURE_WRAP_T, this.REPEAT); //Stretch image to Y position
  ...
  static renderObject(value) {
    KaylaEngine.shaders[value.shaderName]
      .activate()
      .preRender(value.shaderVariables)
      .setCameraMatrix(KaylaEngine.camera.getMatrix(value.renderMatrixName))
      .renderModal(value.preRender());
  }
}
